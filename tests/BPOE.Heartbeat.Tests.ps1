...
Describe "BPOE: Long steps show heartbeat" {
  ...
  foreach ($root in $roots) {
    if (Test-Path $root) {
      $scripts += Get-ChildItem -Recurse -File -Path $root -Include *.ps1 -ErrorAction SilentlyContinue
    }
  }
  ...
  It "wraps long ops with heartbeat: <_.Name>" -ForEach $scripts {
    ...
    if ($hasLongCall) {
      $hasHeartbeat | Should -BeTrue -Because "$($s.FullName) has long ops but no heartbeat"
    } else {
      $true | Should -BeTrue
    }
  }
