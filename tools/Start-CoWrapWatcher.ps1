...
$daemon = Join-Path (Split-Path $MyInvocation.MyCommand.Path -Parent) "CoWrap.WatcherDaemon.ps1"
...
if (Test-Path $pidFile) {
  ...
  if ($watcherPid -and (Get-Process -Id $watcherPid -ErrorAction SilentlyContinue)) {
    Write-Host ("CoWrap watcher already running (PID {0}) — Folder: {1}" -f $watcherPid,$dl); exit 0
  }
}
$args = @("-NoProfile","-File",$daemon,"-Watch",$dl)
$p = Start-Process -FilePath $shell -WindowStyle Hidden -ArgumentList $args -PassThru
$p.Id | Set-Content -Path $pidFile
Write-Host ("CoWrap watcher started. Folder: {0}  (pid: {1})" -f $dl,$p.Id)