...
if (Test-Path $PROFILE) {
  ...
  if (@($errs).Count) {
    $msgs = $errs | ForEach-Object { $_.Message + ' @ ' + $_.Extent.StartLineNumber + ':'
+ $_.Extent.StartColumnNumber }
    $issues.Add('Profile parse errors: ' + ([string]::Join('; ', $msgs)))
...
try {
  ...
  if ($havePrompt) {
    ...
    if ($had) { $global:__OEStatusNext = $prev } else { Remove-Variable -Name __OEStatusNext -Scope Global -ErrorAction SilentlyContinue }
    if (-not ($issues | Where-Object { $_ -like 'prompt invocation error:*' })) { $checks.Add('Prompt invoke: OK') }
...
try {
  ...
  if (Test-Path $repoCiv) {
    ...
    if ($okL -and $okI) { $checks.Add('Workbench launcher/inner: OK') } else { $issues.Add('Workbench scripts missing (launcher/inner)') }
  }
} catch { $issues.Add('workbench probe failed: ' + $_.Exception.Message) }
# Emit
if (@($issues).Count) {
  $lines = @('' )
  $lines += "## BPOE Preflight — $now"
  $lines += '- Result: FAIL'
  $lines += '- Issues:'
  $lines += ($issues | ForEach-Object { '  - ' + $_ })
  $lines += '- Checks:'
  $lines += ($checks | ForEach-Object { '  - ' + $_ })
  Add-BpoeLogLine -Lines $lines
  if (-not $Quiet) { Write-Warning ([string]::Join('; ', $issues)) }
} else {
